 ...
 * @author maozhouge
 */
public class WC206MediumUnhappyFriends {
    public static int unhappyFriends(int n, int[][] preferences, int[][] pairs) {
        ...
        for (int i = 0; i < preferences.length; i++) {
            int[] inner = preferences[i];
            for (int j = 0; j < inner.length; j++) {
                close[i][inner[j]] = inner.length - j;
            }
        }

        int[][] allPairs = new int[pairs.length * 2][2];
        ...
        for (int[] pair : allPairs) {
            ...
            int y = pair[1];
            for (int[] allPair : allPairs) {
                int u = allPair[0];
                int v = allPair[1];

                if (x != u && x != v) {
                    if (close[x][u] > close[x][y] && close[u][x] > close[u][v]) {
                        unhappy[x] = 1;
                    }
                }
            }
